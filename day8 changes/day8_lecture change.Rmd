...
minpair$allmean <- apply(minpair[, 10:17], MARGIN = 1, mean) # give it the data you want it to, here, columns 10-17. 
# made a new column and that column is the mean of each row
head(minpair)
# for accross multiple columns of your data
```
...
results <_ ddply(mydata, c("factor1","factor2"), my.function) # function will apply to that chunk of stuff
```
## `dplyr`
...
### `dplyr` usage
  ...
  - `summarise` = "boil tings down to some kind of summary"
  
```{r}
library("dplyr")
minpair <- mutate(minpair, newcolumn = log(HiSegTok))
head(minpair)
# the same as
...
head(minpair)
head(minpair.grouped) #nothing in dataframe has changed but you get a different output
#clean-up (sometimes, but narely, needed)
...
y <- log(x)
z <- sd(y)
z  # saving a lot of parts

sd(log(1:10)) # hard to read, have to think inside out

1:10 %>% log %>% sd #take those results and then do the next things, and then the next thing. think sequentially.

minpair %>% group_by(Corpus) %>% summarize(mean1 = mean(HiSegTok), mean2 = mean(LoSegTok))
...
minpair %>%
  ...
  summarize(mean1 = mean(HiSegTok), mean2 = mean(LoSegTok)) %>%
  summarize(mean1 = mean(mean1), mean2 = mean(mean))
