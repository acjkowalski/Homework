...
ggplot(myris, aes(Sepal.Length)) + geom_histogram(binwidth = 0.5)

ggplot(myris, aes(Sepal.Length)) + geom_histogram(binwidth = 0.1)

mybaseplot <- ggplot(myris, aes(x = Sepal.Length))
mybaseplot + geom_histogram(binwidth = 0.2)

names(mybaseplot)
str(mybaseplot)

mybaseplot + geom_density(adjust= 0.5, fill = "aliceblue")
```
...
faceting
```{r}
ggplot(myris, aes(x = Sepal.Length)) + geom_histogram() + facet_grid(Species ~ .)

ggplot(myris, aes(x = Sepal.Length)) + geom_histogram() + facet_wrap(~Species, scales = "free") #scale is different, usually don't want this
```
### saving plots
```{r}
pdf("myplot.pdf") # starts a device. opens a file
ggplot(myris, aes(x = ))
dev.off()
